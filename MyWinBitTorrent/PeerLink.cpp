...
CPeerLink::CPeerLink(void) : m_pPeerManager(NULL)
{
}
...
bool CPeerLink::ShouldClose()
{
    ...
    {
        if (m_pPeerManager->GetTorrentTask()->GetTaskStorage()->GetBanedBitSet()->IsSet(i))
        {
            continue;
        }

        if (m_clBitSet.IsSet(i) && !m_pPeerManager->GetTorrentTask()->GetTaskStorage()->GetBitSet()->IsSet(i))
        {
            return false;
        }
    }
...
{
    m_nUploadSpeed = (m_nUploadCount - m_nLastUploadCount) * 1000 / (GetTickCount() - m_llLastCountSpeedTime + 1);
    m_nDownloadSpeed = (m_nDownloadCount - m_nLastDownloadCount) * 1000 / (GetTickCount() - m_llLastCountSpeedTime + 1);
...
void CPeerLink::SendHandShake()
{
    ...
    strncpy(szBuff + 28, (const char *)m_pPeerManager->GetTorrentTask()->GetTorrentFile()->GetInfoHash(), 20);
    strncpy(szBuff + 48, (const char *)m_pPeerManager->GetTorrentTask()->GetPeerID().c_str(), 20);
...
void CPeerLink::SendPieceCancel(unsigned int nPieceIndex, unsigned int nOffset, unsigned int nLen )
{
    char szBuff[17];
    *((unsigned int *)szBuff) = htonl(13);
...
bool CPeerLink::CheckHandshake( string strInfo )
{
    ...
    strMyInfoHash.append((const char*)m_pPeerManager->GetTorrentTask()->GetTorrentFile()->GetInfoHash(), 20);

    if (strInfo != strMyInfoHash)
    {
        return false;
    }
    ...
    m_clBitSet.Alloc(m_pPeerManager->GetTorrentTask()->GetTorrentFile()->GetPieceCount());
...
void CPeerLink::OnConnectClosed()
{
    ...
    m_pPeerManager->GetTorrentTask()->GetTaskStorage()->AbandonPieceTask(m_clPieceRequest.GetPieceIndex());
    m_clPieceRequest.CreatePieceRequestList(-1, 0, 0);
}
...
void CPeerLink::GetNewPieceTask()
{
    ...
    int nPieceIndex = m_pPeerManager->GetTorrentTask()->GetTaskStorage()->GetPieceTask(&m_clBitSet);
    if (nPieceIndex != -1)
    {
        if (!m_bAmInterested)
        {
        ...
        m_clPieceRequest.CreatePieceRequestList(nPieceIndex, m_pPeerManager->GetTorrentTask()->GetTaskStorage()->GetPieceLength(nPieceIndex), REQUEST_BLOCK_SIZE);
    }
    else
    {
        if (m_bAmInterested)
...
{
    if (!m_bAmChoking &&
        m_strSendBuffer.size() == 0 &&
        m_lstPeerPieceRequest.size() > 0)
    {
        PeerPieceRequest stRequest = m_lstPeerPieceRequest.front();
        m_lstPeerPieceRequest.pop_front();

        string strPieceData = m_pPeerManager->GetTorrentTask()->GetTaskStorage()->ReadData(stRequest.nIndex, stRequest.nOffset, stRequest.nLen);
