...
int CTorrentParser::LoadTorrentFile( const char *pTorrentFile )
{
    ...
    if (fp == NULL)
    {
        HandleErrMsg("打开文件失败", __FILE__, WSAGetLastError(), __LINE__);
        return -1;
    }
    ...
    if (m_llFileSize == -1)
    {
        HandleErrMsg("获取文件大小失败", __FILE__, WSAGetLastError(), __LINE__);
        return -1;
    }
    ...
    if (m_lpContent == NULL)
    {
        HandleErrMsg("创建缓冲区失败", __FILE__, WSAGetLastError(), __LINE__);
        return -1;
    }
